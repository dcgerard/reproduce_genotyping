...
for (index in 1:ncol(counts_mat)) {
  ...
  postmean <- c(uout$postmat %*% 0:6)
  dfdat <- data_frame(A = ocounts, a = osize - ocounts, ogeno = factor(uout$ogeno, levels = 0:ploidy),
                      omean = postmean, prob_ok = uout$prob_ok)
  ...
  summaries_mat[index, ] <- c(pcounts, psize - pcounts, uout$seq_error, uout$bias_val,
