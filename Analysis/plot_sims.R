...
## Bias estimates
pl <- ggplot(data = longdat, mapping = aes(x = allele_freq, y = ubias_val)) +
  facet_grid(bias_val ~ od_param) +
  theme_bw() +
  theme(strip.background = element_rect(fill = "white")) +
  xlab("Allele Frequency") +
  ylab("Estimated Bias Parameter") +
  geom_point(size = 0.1) +
  geom_hline(mapping = aes(yintercept = bias_val), lty = 2, color = "red")
pdf(file = "./Output/fig/bias_v_af.pdf", family = "Times", colormodel = "cmyk", height = 8, width = 6.5)
print(pl)
dev.off()

pl_bias <- ggplot(data = longdat, mapping = aes(x = as.factor(od_param), y = ubias_val)) +
  facet_grid(.~bias_val) +
  geom_boxplot(outlier.size = 0.1) +
  theme_bw() +
  theme(strip.background = element_rect(fill = "white"),
